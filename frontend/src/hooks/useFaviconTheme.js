...
export const useFaviconTheme = () => {
  ...
  useEffect(() => {
    // Read the live brand token so the tab icon always matches the app.
    // Falls back to the light-theme primary if the stylesheet has not
    // resolved yet (first paint, or a non-browser environment).
    const token = getComputedStyle(document.documentElement)
      .getPropertyValue('--primary')
      .trim();
    const logoColor = token || '#16130f';
